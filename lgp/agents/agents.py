...
def build_alfred_hierarchical_agent(agent_setup, hparams, device, suffix):
    ...
    if suffix is not None:
        subgoal_model_path = subgoal_model_path[:-11] + '_' + suffix + subgoal_model_path[-11:]
...
def build_alfred_deviant_agent(agent_setup, hparams, device, suffix):
...
def build_demo_replay_agent(agent_setup, hparams, device, suffix):
    from lgp.agents.alfred.demonstration_replay_agent import DemonstrationReplayAgent
...
def get_agent(setup: Hyperparams, hparams: Hyperparams, device=None, suffix=None):
    ...
    return AGENT_BUILDERS[agent_type](agent_setup, hparams, device, suffix)
