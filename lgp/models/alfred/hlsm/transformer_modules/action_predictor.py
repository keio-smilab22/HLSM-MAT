...
class ActionPredictor(nn.Module):
    def __init__(self, dmodel, joint_prob=False, trm_dim=None):
        super().__init__()
    ...
    def build_attn_mask(self, ns, device):
        if self.trm_dim == 0:
            attn_mask = torch.zeros((ns*3, ns*3), device=device, dtype=torch.float32)
            for i in range(3):
                for j in range(3):
                    mask = torch.ones((ns, ns), device=device, dtype=torch.float32)
                    mask = torch.triu(mask, diagonal=1)
                    attn_mask[ns*i:ns*(i+1), ns*j:ns*(j+1)] = mask
            return attn_mask.bool()

        else:
            attn_mask = torch.ones((ns, ns), device=device, dtype=torch.float32)
    ...
    def forward(self, state_embeddings, sentence_embeddings, action_hist_embeddings):
        ...
        if self.trm_dim is None:
            ...
            attn_mask_a = self.build_attn_mask(ns, device)
            attn_mask_b = self.build_attn_mask(ns, device)

            combined_embedding = torch.cat([state_embeddings, sentence_embeddings, action_hist_embeddings], dim=self.trm_dim)
            hidden_embedding = self.transformer_layer_a(combined_embedding, attn_mask=attn_mask_a)
            hidden_embedding = self.transformer_layer_b(hidden_embedding, attn_mask=attn_mask_b)
            combined_embedding = combined_embedding + hidden_embedding

            if self.trm_dim == 0:
                combined_embedding = torch.cat(
                    [combined_embedding[:ns], combined_embedding[ns:2*ns], combined_embedding[2*ns:]], dim=1)
        ...
        if self.joint_prob:
            ...
            act_type_prob = F.softmax(act_type_logits, dim=1)
            act_arg_prob = F.softmax(act_arg_logits, dim=2)
            ...
            act_type_prob = F.softmax(act_type_logits, dim=1)
            act_arg_prob = F.softmax(act_arg_logits, dim=1)
            act_type_logprob = F.log_softmax(act_type_logits, dim=1)
            act_arg_logprob = F.log_softmax(act_arg_logits, dim=1)

        return act_type_prob, act_arg_prob, act_type_logprob, act_arg_logprob